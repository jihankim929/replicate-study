    ...
    for rep in reps:
        ...
        chk = subprocess.run(
            ["ssh", "-o", "BatchMode=yes", "dirac-bei",
             ...
             f"grep -c '^- \\*\\*Context hygiene' /home1/users/Bei/ws/{rep}/CHARTER.md"],
            capture_output=True, text=True).stdout.split()
        appx, pinned, cost, hyg = ((int(chk[0]), int(chk[1]), int(chk[2]), int(chk[3]))
                                   if len(chk) == 4 else (-1, -1, -1, -1))
        ok = (appx == (1 if arm == "gated" else 0)) and pinned == 1 and cost == 1 and hyg == 1
        print(f"  {rep:<7} arm={arm:<8} appA={appx} §3-pinned={pinned} §4-cost={cost} "
              f"§4-hygiene={hyg}  {'OK' if ok else 'MISMATCH'}")
        bad += 0 if ok else 1
